...
class NewsCategoryClassifier:
    ...
    def predict_proba(self, model_input: dict) -> dict:
        ...
        """
        X = list(model_input.values())
        prob_y = self.pipeline.predict_proba(X).reshape(-1,1).tolist()
        prob_y = [round(val[0],4) for val in prob_y]
        return dict(zip(self.labels, prob_y))
    ...
    def predict_label(self, model_input: dict) -> str:
        ...
        """
        return self.pipeline.predict(list(model_input.values()))
...
def predict(request: PredictRequest):
    ...
    news = NewsCategoryClassifier(GLOBAL_CONFIG)
    predicted_proba = news.predict_proba({'X':description})
    predicted_label = news.predict_label({'X':description})[0]
    predict_dict  = {'scores':predicted_proba, 'label':predicted_label}
    end = time.time()
    log_dict = {'timestamp': timestamp, 'request': request_dict, 'prediction':predict_dict, 'latency': 1000*(end-start)}
    logger.debug(f'{log_dict}')
    return PredictResponse(**predict_dict)
